...
Five units. The first two are the ones to get right; the rest are additions on
top of them.

| # | Unit | Hand over | Where |
|---|---|---|---|
| 1 | Schema + `ClipKind` (task 1) | this file; ADR-0006; ADR-0008; `clips/store.rs` | either |
| 2 | `clips/blobs.rs` — write, read, overwrite-and-unlink, orphan sweep (tasks 2, 4) | this file; ADR-0008; `clips/store.rs` | either |
| 3 | Capture, both platforms (task 3) | this file; ADR-0025; ADR-0030; `clips/watch.rs`, `clips/os.rs` | one agent per platform |
| 4 | Paste-back + link classification (tasks 5, 6) | this file; ADR-0022; `clips/paste.rs`, `search/fetch.rs` | either |
| 5 | Palette rows, `!v` preview, Settings (tasks 7, 8) | this file; ADR-0023; `palette/`, the Clipboard settings page | either |

Task 9, the ADR-0008 amendment, is written **with unit 1**, not at the end. The
threat model changes the moment the schema can hold an image, whether or not
anything captures one yet.

Unit 2 is the one to review hardest. Every path that destroys clipboard data goes
through it, and ADR-0008's warning — "the row is deleted" and "the secret is
gone" are different claims — is exactly what a blob file makes easy to get wrong.
It should land with its own tests before unit 3 gives it anything to store.

**Do not start this phase while a `v0.12-macos.md` row is open.** Unit 3 edits
`clips/os.rs` and `clips/watch.rs`, which row 6 is still building.
