...
**Corrected after the build, from the source rather than this summary.**
`resolveWindowsEnvironment` does not read the registry. It spawns PowerShell —
`pwsh.exe`, then `powershell.exe` — with `-NoProfile -NonInteractive` and calls
`[Environment]::GetEnvironmentVariable()`, on a 5 s timeout, then merges a list
of known CLI directories (npm, nodejs, Volta, pnpm, `.local/bin`, `.bun/bin`,
scoop). It then **tests whether `node` resolves**, and only if it does not does it
re-read with the profile loaded, capturing FNM's variables. The `§ Windows`
decision below replaces all of that with one registry read; what it gives up is
the profile pass, and `docs/tbd/v0.11.md` §4 owns that gap.
...
1. [x] **`agents/shellenv.rs` with the pure parts first** — `merge_path_entries`,
...
2. [x] **The Unix probe** — `path_from_login_shell`, `path_from_launchctl`, behind
   `#[cfg(unix)]`, with the timeout-and-kill path.
3. [x] **The Windows probe** — registry read plus `ExpandEnvironmentStringsW`, behind
   `#[cfg(windows)]`.
4. [x] **Cache and invalidation** — `OnceLock`-style storage with an explicit
   `invalidate`, `hydrate()` wired into `lib.rs`'s deferred-init thread.
5. [x] **`probe::resolve` rewritten** to the three-tier search, `extra_dirs()`
   demoted with a comment saying why it is kept.
6. [x] **Re-probe on failure** — `agents/mod.rs` calls `invalidate()` + `hydrate()`
   the first time a previously-resolved Agent stops resolving, then retries once.
7. [x] **Surface it.** Settings → Agents shows which shell answered and how many
...
8. [x] **Row 2's `PATH` executables consume the same hydrated value** —
...
Two decisions were made during the build that this plan did not anticipate.
Both were put to the user and both are deliberate.

**The hydrated `PATH` goes onto every spawned child, not only into resolution.**
The plan stops at finding the binary, which is not enough: a `claude` installed
by `bun add -g` is a shim that re-execs `node`, so a child handed the login
`PATH` fails at its own first step on the exact machine where resolution has just
succeeded. `probe::command` therefore sets `PATH` on every `Command` it builds.
The alternative considered and rejected was `std::env::set_var("PATH", …)` once
at hydration: it would fix every caller for free, including the file walk, but it
is a process-wide mutation, racy against concurrent reads, and unsafe in edition
2024.

**Task 8 was written whole rather than split at the platform line.** The plan
defers it to the Mac. That is not necessary: the macOS arm compiles under
`bun run check:macos` today, so writing both arms now costs nothing and closes
the same hole on Windows, where a `bun add -g rg` was previously invisible to
Row 2 until a reboot. Only the *runtime* verification of the macOS arm waits for
hardware, and `docs/verify/v0.11.md` §B says so.

The shape of `sources/apps/path.rs` changed more than "gains a macOS arm"
suggests. `split_path_var`'s hand-rolled parser was replaced by
`std::env::split_paths`, which already carries the platform separator and
Windows' quoting rule; every existing test passes unchanged against it. The
launchability predicate and the stem both fork by platform, the second because
`file_stem` would offer `python3` for a `python3.11` that is a different binary.
