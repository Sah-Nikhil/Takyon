...
| # | Unit | Hand over | Where | state |
|---|---|---|---|---|
| 0 | First run | this file § Build order | **Mac, human** — not an agent task | **open** |
| 1 | Bench harness → Rust | § Build order, § Testing; `scripts/bench.ts` + the three `.ps1` as the spec; ADR-0003; TBC-0002 | writes on either, verifies on Mac | written, Windows half measured |
| 2 | `tauri.macos.conf.json` | § Build order; ADR-0020; ADR-0028 | either | written |
| 3 | Row 8 — window | ADR-0028; ADR-0003; § Traps; `window.rs` | Mac | written **except the `NSPanel` swap** (tbd §9) |
| 4 | Row 7 — launch | row 7; ADR-0026; `launch.rs`, `entry.rs`'s `LaunchTarget` | Mac | written |
| 5 | Row 3 — icons | row 3; ADR-0026; ADR-0012; `icons.rs` | Mac | written |
| 6 | PATH hydration | `v0.11-path-hydration.md` alone — it names its own reads | **Windows first**, both platforms | done, Windows half proven |
| 7 | Row 2 remainder | row 2; `v0.11-path-hydration.md` (done first); `sources/apps.rs`, `apps/bundles.rs` | Mac | written |
| 8 | Row 4 — Spotlight | ADR-0027; ADR-0007 (what it replaces); `index/mod.rs` | Mac | written |
| 9 | Row 5 — `URLSession` | ADR-0029; ADR-0019; TBC-0013; `search/fetch.rs` | Mac | written |
| 10 | Row 6 — clipboard | ADR-0030; ADR-0006; ADR-0008; ADR-0025; `clips/` | Mac | written, all four parts |
| 11 | Rows 10, 11, 12 | those rows; ADR-0026 | Mac | written |
| 12 | Cmd+Space onboarding | § Onboarding, § Permissions; `hotkey.rs`, `firstrun.rs` | Mac | **mechanism written, first-run screen open** (tbd §15) |
| 13 | Uninstall + data removal | § Uninstall; `identity.rs` | either | written |
| 14 | `docs/verify/macos.md` | this file, whole | written *as* units land, not after | §A–M written, none run |

**"Written" means it compiles for `aarch64-apple-darwin` and nothing more.**
Only units 1 and 6 have had a line executed, and only their Windows halves. The
gate on Windows is `bun run check:macos`; the gate on the Mac is
`cargo test -p takyon`, and it has never been run there.

**Two units are still open to an agent**, and neither needs a Mac to write:
unit 12's blocking first-run screen, and unit 3's `NSPanel` swap — though that
one should not be written at all until `docs/verify/macos.md` §E.1 says whether
it is needed.
