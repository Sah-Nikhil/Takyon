...
/// The tooltip, which is where the application walk reports from v0.9.
///
/// The Palette stopped saying it: there it was a row competing with the Bang's
/// own (`docs/tbd/v0.9.md` §10). Here it costs nothing and is where a background
/// job belongs.
pub fn tooltip(indexing: bool) -> String {
    if indexing {
        format!("{DISPLAY_NAME} — indexing applications…")
    } else {
        DISPLAY_NAME.to_string()
    }
}

/// Update the tooltip to match the walk. Silent when there is no tray.
pub fn set_indexing(app: &AppHandle, indexing: bool) {
    if let Some(icon) = app.tray_by_id(TRAY_ID) {
        let _ = icon.set_tooltip(Some(tooltip(indexing)));
    }
}
...
pub fn build(app: &AppHandle) -> tauri::Result<()> {
    ...
    let mut builder = TrayIconBuilder::with_id(TRAY_ID)
        // The walk is running at this point: the tray says so until it lands.
...
#[cfg(test)]
mod tests {
    /// The tooltip names the product first: it is what a person hovers to find
    /// out what this icon is, and the walk is the second half of the sentence.
    #[test]
    fn v0_9_the_tooltip_says_when_the_walk_is_running() {
        let busy = super::tooltip(true);
        assert!(busy.starts_with(super::DISPLAY_NAME));
        assert!(busy.contains("indexing applications"));
        assert_eq!(super::tooltip(false), super::DISPLAY_NAME);
    }
